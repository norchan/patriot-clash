...
import { getEnemyById, getRandomEnemy, getEnemiesForParty, republicanEnemies, democratEnemies, ENEMY_3D_IDS } from '@/config/enemies'
...
// SHARED SPRITE BATTLE — the one 12-second showdown, rendered for BOTH the
// signed-in map fights and the free guest fights. `guest` flips off the
// account bits (no server settle, no FP/capture kept) and swaps the exit ramp
// for sign-up, so guests get the EXACT same 3D-rig, rock/firecracker fight as
// real players. Authed page mounts <SpriteBattle/>, guest page <SpriteBattle guest/>.
...
const FOE_THROWS: Record<string, { emoji: string; label: string; img?: string }> = {
  ...
  crazy_liberal:   { emoji: '📄', label: 'Pink Slip' },
  crying_liberal:  { emoji: '💧', label: 'Tear Flood' },
  protestor:       { emoji: '🥫', label: 'Can of Soup' },
...
export default function SpriteBattle({ guest = false }: { guest?: boolean }) {
  const router = useRouter()
  const searchParams = useSearchParams()
  const { profile, refetch } = useProfile()
  // guest links from /play use ?e=; the map uses ?enemy=
  const enemyId = searchParams.get('enemy') ?? searchParams.get('e')
  const spawnId = guest ? null : searchParams.get('spawn')
  const backTo = guest ? '/play' : '/map'
  const allEnemies = [...republicanEnemies, ...democratEnemies]
  ...
  const playerLevel = guest ? 1 : fighterLevel(profile?.total_battles_won ?? 0)
  ...
  useEffect(() => {
    ...
    // guest has no party — face a random side; a signed-in player faces the
    // opposite party
    const opponentParty = guest
      ? (Math.random() < 0.5 ? 'republican' : 'democrat')
      : profile!.party === 'republican' ? 'democrat' : 'republican'
  ...
  }, [enemyId, profile, guest]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  async function recordBattle(result: 'victory' | 'defeat' | 'fled', moves: any[]) {
    // guests keep nothing — the win is real, the loot is the sign-up pitch.
    // Report the FP a real player WOULD earn so the victory screen can tease it.
    if (guest) return { captured: false, fp_earned: enemy?.fpReward ?? 0 }
    try {
  ...
  const partyColor = guest ? '#7c3aed' : profile!.party === 'republican' ? '#dc2626' : '#2563eb'
  ...
  // guest end-screen ramp: sign-up + fight-another + back-to-map
  const another = () => {
    const e = allEnemies[Math.floor(Math.random() * allEnemies.length)]
    router.replace(`/play/battle?e=${e.id}`)
  }
  const guestButtons = (
    <>
      <button onClick={() => router.push('/sign-up')}
        style={{ width: '100%', padding: '14px 0', border: 'none', borderRadius: 12, color: 'white', fontWeight: 900, fontSize: 16, cursor: 'pointer', background: 'linear-gradient(90deg, #2563eb, #7c3aed, #dc2626)' }}>
        ⚔️ Sign up — free
      </button>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, width: '100%' }}>
        <button onClick={another} style={{ padding: '11px 0', borderRadius: 12, border: '1px solid rgba(255,255,255,0.14)', background: 'rgba(255,255,255,0.06)', color: '#d1d5db', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>Fight another</button>
        <button onClick={() => router.push('/play')} style={{ padding: '11px 0', borderRadius: 12, border: '1px solid rgba(255,255,255,0.14)', background: 'rgba(255,255,255,0.06)', color: '#d1d5db', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>Back to map</button>
      </div>
    </>
  )
  ...
  return (
    ...
    >
      {/* guests get the full-page ad gate on the first fight of the session */}
      {guest && <GuestAdGate gateKey="battle" />}
      ...
      {phase === 'fighting' && (
        <button
          ...
          onClick={async () => { S.current.over = true; await recordBattle('fled', movesRef.current); router.push(backTo) }}
          style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
          <span style={{ color: 'white', fontWeight: 800, fontSize: 13 }}>{guest ? 'You' : profile!.username} <span style={{ color: '#9ca3af', fontWeight: 700, fontSize: 11 }}>Lv.{playerLevel}</span></span>
          {guest
            ? <span style={{ color: '#a78bfa', fontSize: 11, fontWeight: 800 }}>👻 GUEST</span>
        ...
        <div style={{ position: 'absolute', inset: 0, zIndex: 60, background: 'rgba(4,8,16,0.78)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ width: '100%', maxWidth: 360, display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 44 }}>{guest ? '🏆' : captured ? '🎯' : '💨'}</div>
              <h2 style={{ color: 'white', fontWeight: 900, fontSize: 24, margin: '4px 0' }}>
                {guest ? `You beat ${enemy.name}!` : captured ? `${enemy.name} Captured!` : `${enemy.name} slipped away!`}
              </h2>
              <p style={{ color: '#4ade80', fontSize: 14, fontWeight: 700, margin: 0 }}>
                {guest ? `Players earn +${fpEarned} FP + a capture chance for this` : `+${fpEarned} FP earned!`}
              </p>
            </div>
            <div style={{
              ...
              <img src={enemy.image} alt={enemy.name}
                style={{ width: 110, height: 110, objectFit: 'contain', filter: `drop-shadow(0 0 14px ${tierColor}66)${guest || !captured ? ' grayscale(0.7)' : ''}` }} />
              <p style={{ color: 'white', fontWeight: 800, fontSize: 14, margin: '6px 0 0' }}>
                {guest ? 'Sign up free to keep your winnings' : captured ? '📦 Added to your collection' : 'Beaten — but not caught'}
              </p>
              <p style={{ color: '#9ca3af', fontSize: 11, margin: '2px 0 0' }}>
                {guest
                  ? 'Real players capture characters, earn FP, level up, and hit harder.'
                  : captured
      ...
      {phase === 'defeat' && (
        <div style={{ position: 'absolute', inset: 0, zIndex: 60, background: 'rgba(4,8,16,0.78)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ width: '100%', maxWidth: 360, display: 'flex', flexDirection: 'column', gap: 10, alignItems: 'center' }}>
            ...
            <p style={{ color: '#9ca3af', fontSize: 13, margin: '0 0 12px', textAlign: 'center' }}>
              {guest ? 'Real players level up, gear up, and hit harder. Sign up free and come back swinging.' : `${enemy.name}'s ${theme.label} was too much!`}
            </p>
            {guest ? guestButtons : (
              <button onClick={() => router.push('/map')}
                style={{ width: '100%', padding: '12px 0', background: '#1f2937', border: '1px solid #374151', borderRadius: 12, color: 'white', fontWeight: 700, fontSize: 15, cursor: 'pointer' }}>
