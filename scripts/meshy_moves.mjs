// Generate extra move clips (kick, haymaker) for the 12 fist-rebuilt fighter
// rigs → public/models/<tag>_{kick,haymaker}.glb (texture-compressed).
// Resumable. Run in background: node scripts/meshy_moves.mjs
import fs from 'fs'
...
// tag -> rig_task_id (from the fist rebuild log)
const RIGS = [
...
const ACTIONS = [[207, 'kick'], [96, 'haymaker']]
  ...
  for (const [action, name] of ACTIONS) {
    ...
    try {
      ...
      for (let i = 0; i < 120; i++) {
        ...
        process.stdout.write(`\r  ${tag}_${name}: ${s.status} ${s.progress ?? 0}%   `)
        await sleep(5000)
      }
      ...
      console.log(`\n${tag}_${name}: saved (${fs.statSync(dest).size})`); results.done.push(`${tag}_${name}`)
    } catch (e) { console.error(`\n${tag}_${name} FAILED: ${e.message}`); results.failed.push(`${tag}_${name}`) }
  }
}
console.log('MOVES COMPLETE:', JSON.stringify(results))
