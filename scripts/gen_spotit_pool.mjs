...
const MIN_SCORE = 26   // mean RGB delta a circle must produce to count as findable
const MAX_POOL = 40    // cap per scene
const RADII = [42, 56]
const STEP = 44        // grid step for candidate centers
...
for (const scene of SCENES) {
  ...
  // precompute per-pixel delta under a 120° hue rotation
  ...
  for (const r of RADII) {
    for (let cy = r + 8; cy < H - r - 8; cy += STEP) {
      for (let cx = r + 8; cx < W - r - 8; cx += STEP) {
        ...
        const score = sum / n
        if (score >= MIN_SCORE) cands.push({ x: cx, y: cy, r, score })
      }
    }
  }
