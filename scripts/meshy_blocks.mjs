// Block1 (138) held cover pose for all 12 fighter variants →
// public/models/<tag>_block.glb (texture-compressed). Render-approved clip:
// steady forearms-up block, frozen at ~1.2s in-game. Resumable.
import fs from 'fs'
...
for (const [tag, rig] of RIGS) {
  const dest = `public/models/${tag}_block.glb`
  if (fs.existsSync(dest)) { console.log(`SKIP ${tag}`); continue }
  try {
    const r = await fetch(`${API}/animations`, { method: 'POST', headers: H, body: JSON.stringify({ rig_task_id: rig, action_id: 138 }) })
    ...
    for (let i = 0; i < 120; i++) {
      ...
      process.stdout.write(`\r  ${tag}_block: ${s.status} ${s.progress ?? 0}%   `)
      await sleep(5000)
    }
    ...
    console.log(`\n${tag}_block saved (${fs.statSync(dest).size})`); results.done.push(tag)
  } catch (e) { console.error(`\n${tag}_block FAILED: ${e.message}`); results.failed.push(tag) }
}
