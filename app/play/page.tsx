...
// GUEST GAME WORLD — the in-game map for the signed-out, anchored at Cahokia
// (the ancient capital across the river from St. Louis). Real halls, sprites
// to fight (guest battles), and the arcade — halls and players route to
// sign-up. Signed-in players get bounced to their real map.

const CAHOKIA = { lat: 38.6551, lng: -90.0615, zoom: 12.2 }
...
export default function GuestPlayPage() {
  ...
  function drawWorld(m: mapboxgl.Map, halls: any[], players: any[]) {
    ...
    // halls — public, but capturing takes an account
    ...
    // sprites — guests CAN fight these
    const c = m.getCenter()
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const distMi = 0.6 + seededRand(`d${i}|${c.lng.toFixed(1)}`) * 3.5
      const lat = c.lat + (distMi / 69) * Math.sin(angle)
      const lng = c.lng + (distMi / (69 * Math.cos(c.lat * Math.PI / 180))) * Math.cos(angle)
      const el = document.createElement('div')
      el.style.cursor = 'pointer'
      el.innerHTML = `
      ...
      el.addEventListener('click', () => router.push(`/play/battle?e=${e.id}`))
      markersRef.current.push(new mapboxgl.Marker({ element: el }).setLngLat([lng, lat]).addTo(m))
    }

    // the arcade — guests welcome (ads first)
    const arcadeEl = document.createElement('div')
    arcadeEl.style.cursor = 'pointer'
    arcadeEl.innerHTML = `
      <div style="display:flex;flex-direction:column;align-items:center;">
        <img src="/arena.png" style="display:none" />
        <div style="width:54px;height:54px;border-radius:14px;overflow:hidden;border:2.5px solid #facc15;box-shadow:0 0 12px #facc1588;background:#101828;">
          <img src="/arcade/slots.jpg" style="width:100%;height:100%;object-fit:cover;" />
        </div>
        <div style="background:rgba(0,0,0,0.85);border:1.5px solid #facc15;border-radius:7px;padding:1px 6px;color:white;font-size:10px;font-weight:700;margin-top:2px;">🕹️ Arcade</div>
      </div>`
    arcadeEl.addEventListener('click', () => router.push('/play/arcade'))
    markersRef.current.push(
      new mapboxgl.Marker({ element: arcadeEl, anchor: 'bottom' })
        .setLngLat([c.lng + 0.008, c.lat + 0.004]).addTo(m))
  }

  return (
    <div className="relative h-screen overflow-hidden max-w-md mx-auto bg-gray-950">
      {/* w-full h-full is load-bearing — mapbox CSS overrides `absolute` */}
      ...
      <div className="absolute top-0 left-0 right-0 z-20 px-4 pt-4 flex items-center justify-between pointer-events-none">
        <button onClick={() => router.push('/')}
          className="bg-black/80 backdrop-blur rounded-xl px-3 py-2 pointer-events-auto text-white font-black text-sm">
          🏛️ PoliticsGo
        </button>
      ...
      <div className="absolute bottom-6 left-4 right-4 z-20">
        <div className={`bg-gray-900/95 backdrop-blur rounded-2xl border border-gray-700 p-4 shadow-2xl transition-transform ${ctaPulse ? 'scale-95' : ''}`}>
          <p className="text-white font-bold text-sm text-center">👻 Guest mode — Cahokia, IL</p>
          <p className="text-gray-400 text-xs text-center mt-1">
            Tap a sprite to fight it · tap the arcade to play. Sign up to capture halls and keep your FP.
          </p>
          <button onClick={toSignUp}
            className="w-full mt-3 py-3 rounded-xl font-black text-white text-base transition active:scale-95"
